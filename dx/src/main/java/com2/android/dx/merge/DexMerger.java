...
import com2.android.dex.ClassData;
import com2.android.dex.ClassDef;
import com2.android.dex.Code;
    ...
    private SortableType[] getMainDexSortedTypes() {
        ...
        for (int i = 0; i < dexes.length; i++) {
            if (isCodeLibDexFile(dexes[i])) {
                // Logger.info(String.format("Skipping CodeLib because of small MethodCount: %d", dexes[i].methodIds().size()));
                continue;
            }
            readSortableTypes(sortableTypes, dexes[i], indexMaps[i]);
        }
